...
export class Homework {
    ...
    // ### 실행하는 방법
    //
    // * (git 저장소)/ComputerGraphics/webserver.bat 를 실행하세요.
    // * 웹브라우저에서 http://127.0.0.1:8080/BuildMesh1/ 를 여세요.
    // * 코드를 수정하고 F5 키 혹은 Ctrl+R 키로 새로고침하세요.
    // * 에디터로는 Visual Studio Code를 추천합니다.
    //
    //
    // ### 참고
    //
    ...
    //   로컬 커밋을 하며 진행하는 것을 추천합니다.
    //
    // * 보통은 3차원 벡터 p로부터 x, y, z 좌표를 꺼낼 때 p.X, p.Y, p.Z 등의 표현을 사용하지만
    //   이 과제에서 사용하는 라이브러리에서는 p[0], p[1], p[2] 를 사용합니다. 놀라지 마세요~
    // 
    // * 정육면체에는 꼭지점이 8개 있지만, 이 과제에서는 꼭지점을 더 많이 등록해야 합니다.
    //   왜냐하면 같은 꼭지점이라도 어느 삼각형에 속해 있느냐에 따라 색을 다르게 지정해야 하기 때문입니다.
    //   적어도 24개의 꼭지점을 등록해야 할 것입니다.
